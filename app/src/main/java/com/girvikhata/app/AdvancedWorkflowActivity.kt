        ...
        setContent {
            MaterialTheme {
                AdvancedRoot(
                    ...
                    executeWrite = coordinator::execute,
                    close = ::finish,
                )
            }
        }
...
private fun AdvancedRoot(
    ...
    executeWrite: (VerifiedBusinessWriteRequest) -> VerifiedBusinessWriteResult,
    close: () -> Unit,
) {
    ...
    Column(
        ...
        Row(
            ...
            Row {
                TextButton(onClick = ::refresh) { Text("Refresh Data") }
                TextButton(onClick = close) { Text("Close") }
            }
        }
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(
                onClick = { tab = AdvancedTab.MULTI_ITEM },
                modifier = Modifier.weight(1f),
                colors = ButtonDefaults.buttonColors(containerColor = if (tab == AdvancedTab.MULTI_ITEM) Color(0xFF5146B8) else Color.Gray),
            ) { Text("Multi Item") }
            Button(
                onClick = { tab = AdvancedTab.CUSTOM_PAYMENT },
                modifier = Modifier.weight(1f),
                colors = ButtonDefaults.buttonColors(containerColor = if (tab == AdvancedTab.CUSTOM_PAYMENT) Color(0xFF138A4A) else Color.Gray),
            ) { Text("Custom Split") }
        }
        when (tab) {
            AdvancedTab.MULTI_ITEM -> MultiItemEntry(snapshot, catalog) { customer, girvi ->
                val result = executeWrite(
                    VerifiedBusinessWriteRequest(
                        ...
                        title = "Advanced multi-item girvi ${girvi.girviNumber}",
                    ),
                )
                refresh()
                result
            ...
            AdvancedTab.CUSTOM_PAYMENT -> CustomSplitPayment(snapshot, catalog) { updated ->
                ...
                val result = executeWrite(
                    VerifiedBusinessWriteRequest(
                        ...
                        title = "Advanced custom payment ${payment.receiptNumber}",
                    ),
                )
                refresh()
                result
...
@Composable
private fun MultiItemEntry(
    snapshot: AppSnapshot,
    catalog: MasterCatalog,
    save: (CustomerRecord, GirviRecord) -> VerifiedBusinessWriteResult,
) {
    ...
    LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            OutlinedButton(
                onClick = {
                    runCatching {
                        ...
                        val metadata = listOf(
                            "Unit: ${selectedUnit?.name ?: "manual"}",
                            "Locker: ${selectedLocker?.name ?: "not selected"}",
                            itemNote.trim(),
        ...
        item {
            Button(
                onClick = {
                    runCatching {
                        ...
                        require(name.length >= 2) { "Customer required" }
                        val items = MasterWorkflowTransactions.validateItems(drafts)
                        val amount = MoneyInput.rupeesToPaise(principal)
                        val rate = selectedPlan?.rateBasisPoints
                            ?: ((manualRate.toDoubleOrNull() ?: error("Valid rate required")) * 100).roundToLong().toInt()
                        ...
                        val result = save(customer, girvi)
                        drafts.clear()
                        customerName = ""
                        mobile = ""
                        address = ""
                        principal = ""
                        message = "Saved ${girvi.girviNumber} • TX ${result.transactionId.take(8)}"
            ...
            ) { Text("Multi-Item Girvi Save Karein") }
        }
    }
}

@Composable
private fun CustomSplitPayment(
    snapshot: AppSnapshot,
    catalog: MasterCatalog,
    save: (GirviRecord) -> VerifiedBusinessWriteResult,
) {
    ...
    LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        itemsIndexed(active) { _, g ->
            TextButton(onClick = { girviId = g.id }) {
                Text(if (girviId == g.id) "✓ ${g.girviNumber} • ${g.customerName}" else "${g.girviNumber} • ${g.customerName}")
            }
        }
                    ...
                    runCatching {
                        ...
                        val customSplit = if (custom) {
                            ...
                            MasterWorkflowTransactions.validateCustomPayment(
                                CustomPaymentDraft(
                                    rupeesOrZero(principal),
                                    rupeesOrZero(interest),
                                    rupeesOrZero(charges),
                                ),
                        ...
                        val result = save(updated)
                        amount = ""
                        principal = ""
                        interest = ""
                        charges = ""
                        note = ""
                        message = "Saved ${updated.payments.last().receiptNumber} • TX ${result.transactionId.take(8)}"
                    }.onFailure { message = it.message ?: "Payment save failed" }
                },
                enabled = active.isNotEmpty() && modes.isNotEmpty(),
                modifier = Modifier.fillMaxWidth(),
                colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF138A4A)),
            ) { Text("Payment Save Karein") }
        }
    }
}
...
private fun MasterChoices(entries: List<MasterEntry>, selected: String?, select: (String) -> Unit) {
    if (entries.isEmpty()) {
        Text("Koi active option nahi", color = Color.Gray)
        return
    }
    entries.forEach { entry ->
        TextButton(onClick = { select(entry.id) }) {
            Text(if (selected == entry.id) "✓ ${entry.name}" else entry.name)
        }
    }
}
...
private fun TextChoices(values: List<String>, selected: String, select: (String) -> Unit) {
    values.forEach { value ->
        TextButton(onClick = { select(value) }) {
...
private fun rupeesOrZero(value: String): Long = if (value.isBlank()) 0L else MoneyInput.rupeesToPaise(value)
